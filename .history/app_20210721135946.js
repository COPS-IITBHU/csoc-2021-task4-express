...
app.post("/delete",(req,res)=>{
  Book.findOne({title:req.body.title,genre:req.body.genre,author:req.body.author},function(err,foundBook){
    if(err){
      ...
      BookCopy.findOne({borrow_data:req.body.borrow_data,borrower:req.user.username,book:foundBook},function(err,foundBookCopy){
          if(err){
            console.log(err);
          }
          else{
              var updateUserArr;
              User.findOne({username:foundBookCopy.borrower},function(err,foundUser){
                  if(err){
                    console.log(err);
                  }
                  else{
                    updateUserArr=User.loaned_books;
                    updateUserArr=updateUserArr.filter(cpy=>cpy!==foundBookCopy);
                    User.updateOne(foundUser,{
                      ...
                      if(err){
                        ...
                        res.render("./views/loaned_books.ejsk",books:updateUserArr);
                      }
                    });
                  }
