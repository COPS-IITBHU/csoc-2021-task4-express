...
var issueBook = (req, res) => {
    ...
    .then((book) => {
        title=book.title;
        console.log(title);
        BookCopy.find({status:true,book:title},async ((err,List) => {
            if(err) {
                console.log(err);
            }
            else{
                if(List!==[]) {
                     List[0].updateOne({status:false,borrow_data:new Date().toLocaleString(),borrower:req.user.username});
                    User.find({username:req.user.username},((err,updateUser)=>{
                        if(err){
                            console.log(err);
                        }else{
                            updatedArr=updateUser.loaned_books+List[0];
                            updateUser.updateOne({loaned_books:updatedArr});
                        }
                    }));
